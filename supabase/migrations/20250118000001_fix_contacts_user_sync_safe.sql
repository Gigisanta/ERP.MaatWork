-- Migración crítica para reparar error de creación de contactos (versión segura)
-- Fecha: 2025-01-18
-- Problema: user_id en contacts no existe en public.users (violación FK)
-- Solución: Sincronización segura auth.users <-> public.users

-- 1. VERIFICAR Y CORREGIR ESTRUCTURA DE TABLA USERS
-- Primero, verificar si la tabla users ya existe
DO $$
BEGIN
    ...
    -- Agregar constraint UNIQUE en email si no existe
    IF NOT EXISTS (
...
RETURNS TRIGGER AS $$
BEGIN
    -- Insertar o actualizar usuario en public.users de forma segura
    ...
    ON CONFLICT (id) DO UPDATE SET
        email = NEW.email,
        name = COALESCE(NEW.raw_user_meta_data->>'name', EXCLUDED.name, split_part(NEW.email, '@', 1)),
        updated_at = NOW()
...
-- Sincronizar usuarios existentes de auth a public de forma segura
DO $$
DECLARE
    auth_user RECORD;
BEGIN
    -- Iterar sobre usuarios de auth que no están en public
    FOR auth_user IN 
    ...
    LOOP
        ...
        EXCEPTION 
            WHEN unique_violation THEN
                -- Si hay conflicto de email, actualizar el registro existente con el ID correcto
                UPDATE public.users 
                SET id = auth_user.id,
                    name = COALESCE(auth_user.raw_user_meta_data->>'name', name),
                    updated_at = NOW()
...
COMMENT ON FUNCTION sync_auth_user_to_public() IS 'Sincroniza automáticamente usuarios de auth.users a public.users manejando conflictos';
...
-- Migración completada: fix_contacts_user_sync_safe
-- Descripción: Reparación crítica del error de creación de contactos con manejo seguro de conflictos
-- Fecha: 2025-01-18