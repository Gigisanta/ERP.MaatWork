...
export async function handleUpload(req: Request, res: Response) {
  try {
    ...
    const rowsWithAtLeastOneIdentifier =
      parsedRows.length - validation.stats.rowsMissingIdentifiers;
    req.log?.info?.(
      {
        fileId: fileRow.id,
    ...
    if (
      fileMetadata.fileType === 'monthly' &&
      fileMetadata.reportMonth &&
      fileMetadata.reportYear
    ) {
