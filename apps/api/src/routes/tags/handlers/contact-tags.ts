  ...
  try {
    ...
    for (const contactId of contactIds) {
      ...
      if (hasAccess) {
        ...
        req.log.warn({ tagId: id, contactId, userId, userRole }, 'user attempted to add tag to inaccessible contact');
      }
    }
...
router.delete('/:id/contacts/:contactId', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!hasAccess) {
      req.log.warn({ tagId: id, contactId, userId, userRole }, 'user attempted to remove tag from inaccessible contact');
...
router.get('/contacts/:id', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!hasAccess) {
      req.log.warn({ contactId: id, userId, userRole }, 'user attempted to list tags for inaccessible contact');
      return res.status(404).json({ error: 'Contact not found' });
    }
...
router.put('/contacts/:id', 
  ...
  try {
    ...
    const hasAccess = await canAccessContact(userId, userRole, id);
    if (!hasAccess) {
      req.log.warn({ contactId: id, userId, userRole }, 'user attempted to update tags for inaccessible contact');
      return res.status(404).json({ error: 'Contact not found' });
    }
...
router.get('/contacts/:contactId/tags/:tagId',
  ...
  try {
    ...
    if (!hasAccess) {
      req.log.warn({ contactId, tagId, userId, userRole }, 'user attempted to get contact tag for inaccessible contact');
      return res.status(404).json({ error: 'Contact not found' });
    }
...
router.put('/contacts/:contactId/tags/:tagId',
  ...
  try {
    ...
    req.log.info({ contactId, tagId, monthlyPremium, policyNumber }, 'PUT /contacts/:contactId/tags/:tagId - start');
    ...
    if (!hasAccess) {
      req.log.warn({ contactId, tagId, userId, userRole }, 'user attempted to update contact tag for inaccessible contact');
      return res.status(404).json({ error: 'Contact not found' });
    }
