...
import { db, teamGoals, teamMembership } from '@maatwork/db';
import { teams } from '@maatwork/db/schema';
 ...
 */
export const getTeamGoals = createRouteHandler(async (req: Request) => {
 ...
 */
export const updateTeamGoal = createRouteHandler(async (req: Request) => {
  const teamId = validateUuidParam(req.params.id, 'teamId');
  ...
  if (!isManager) {
    throw new HttpError(403, 'Only managers can update goals');
  }
