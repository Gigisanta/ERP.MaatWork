...
import { db, teamMembership } from '@maatwork/db';
 ...
 */
export const getUnassignedOrStalledLeads = createRouteHandler(async (req: Request) => {
 ...
 * Bulk reassign leads
 */
export const reassignLeads = createRouteHandler(async (req: Request) => {
  const teamId = validateUuidParam(req.params.id, 'teamId');
  ...
  if (!member) {
    throw new HttpError(400, 'New advisor is not in the team');
  }
