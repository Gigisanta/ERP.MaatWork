 ...
 */
export const listMembershipRequests = createRouteHandler(async (req: Request) => {
  const userId = req.user!.id;
  const userRole = req.user!.role;

  // Solo managers y admins pueden ver solicitudes
  if (userRole !== 'manager' && userRole !== 'admin') {
    throw new HttpError(403, 'Access denied. Only managers can view membership requests.');
  }
 ...
 */
export const approveAllRequests = createRouteHandler(async (req: Request) => {
  const currentUserId = req.user!.id;
  const currentRole = req.user!.role;

  if (currentRole !== 'manager' && currentRole !== 'admin') {
    throw new HttpError(403, 'Access denied. Only managers or admins can approve requests.');
  }
  ...
  return { ok: true, approved, teamId: managerTeam.id };
});

/**
 * POST /teams/membership-requests/:id/approve - Aprobar solicitud de membresía
 */
export const approveRequest = createRouteHandler(async (req: Request) => {
  let id: string;
  try {
    id = validateUuidParam(req.params.id, 'requestId');
  } catch (err) {
    throw new HttpError(400, err instanceof Error ? err.message : 'Invalid request ID format');
  }
  const userId = req.user!.id;
  const userRole = req.user!.role;
  ...
  if (userRole !== 'manager' && userRole !== 'admin') {
    throw new HttpError(403, 'Access denied. Only managers can approve requests.');
  }
  ...
  if (request.status !== 'pending') {
    ...
    return { approved: request.status === 'approved', alreadyResolved: true };
  }

  // Verificar que el manager actual es el destinatario de la solicitud
  if (userRole === 'manager' && request.managerId !== userId) {
    throw new HttpError(403, 'Access denied. You can only approve requests for your team.');
  }
 ...
 */
export const rejectRequest = createRouteHandler(async (req: Request) => {
  let id: string;
  try {
    id = validateUuidParam(req.params.id, 'requestId');
  } catch (err) {
  ...
  if (userRole !== 'manager' && userRole !== 'admin') {
    throw new HttpError(403, 'Access denied. Only managers can reject requests.');
  }
  ...
  // Idempotencia: si ya fue resuelta, devolver 200
  if (request.status !== 'pending') {
    return { rejected: request.status === 'rejected', alreadyResolved: true };
  }

  // Verificar que el manager actual es el destinatario de la solicitud
  if (userRole === 'manager' && request.managerId !== userId) {
    throw new HttpError(403, 'Access denied. You can only reject requests for your team.');
  }
 ...
 */
export const deleteRequest = createRouteHandler(async (req: Request) => {
  let id: string;
  try {
    id = validateUuidParam(req.params.id, 'requestId');
  } catch (err) {
  ...
  if (userRole !== 'manager' && userRole !== 'admin') {
    throw new HttpError(403, 'Access denied. Only managers can delete requests.');
  }
  ...
  const [request] = await dbi
    ...
    .where(eq(teamMembershipRequests.id, id))
    .limit(1);
  if (!request) throw new HttpError(404, 'Membership request not found');

  // Managers can only delete their own requests; admins can delete any
  if (userRole === 'manager' && request.managerId !== userId) {
    throw new HttpError(403, 'Access denied. You can only delete requests for your team.');
  }
