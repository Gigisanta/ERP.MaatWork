...
router.patch(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.params as { id: string };
      const { role } = req.body as { role: 'admin' | 'manager' | 'advisor' | 'owner' };
    ...
    try {
      ...
      if (!user.passwordHash) {
        return res
          .status(400)
          .json({
            error: 'No se puede cambiar contraseña: usuario no tiene contraseña configurada',
          });
      }
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!email || !fullName || !password || !role) {
        return res
          .status(400)
          .json({ error: 'Email, nombre completo, contraseña y rol son requeridos' });
      }
      ...
      if (!['advisor', 'manager', 'admin', 'owner', 'staff'].includes(role)) {
...
// AI_DECISION: Endpoint separado de reject para eliminar usuarios activos
// Justificación: Permite a admins eliminar cualquier usuario con validaciones de seguridad
// Impacto: Completa el CRUD de usuarios con protecciones contra auto-eliminación y último admin
// ==========================================================
router.delete(
  '/:id',
  requireAuth,
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      req.log.error({ err, userId: req.params.id }, 'failed to delete user');
      next(err);
    }
  }
);
