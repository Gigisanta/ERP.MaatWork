...
router.post(
  ...
  createAsyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      if (!newContact) {
        throw new Error('Contact creation failed - no contact returned from database');
      }
      ...
      if (validatedPipelineStageId && userId && userId !== '00000000-0000-0000-0000-000000000001') {
        try {
          ...
          req.log.error({ err: historyError }, 'failed to register pipeline stage history');
        }
      }

      const duration = Date.now() - startTime;
      req.log.info({ duration, contactId: newContact.id }, 'Creación de contacto exitosa');
