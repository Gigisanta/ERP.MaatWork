...
// ==========================================================
router.put('/:id', requireAuth, requireRole(['manager', 'admin']), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
router.delete('/:id', requireAuth, requireRole(['manager', 'admin']), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (tag.isSystem) {
      return res.status(403).json({ error: 'Cannot delete system tag' });
    }
...
router.post('/:id/contacts', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const values = contactIds.map(contactId => ({ tagId: id, contactId }));
    ...
    req.log.info({ tagId: id, count: contactIds.length }, 'tag assigned to contacts');
    res.json({ data: { assigned: contactIds.length } });
  } catch (err) {
    if (err instanceof z.ZodError) {
      return res.status(400).json({ error: 'Validation error', details: err.errors });
    }
...
// ==========================================================
// PUT /contacts/:id/tags - Actualizar etiquetas de un contacto
// ==========================================================
...
router.post('/segments/:id/refresh', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Por ahora limpiar y recalcular
    await db()
      .delete(segmentMembers)
      .where(eq(segmentMembers.segmentId, id));

    // Mock: agregar algunos contactos
    const matchedContactIds: string[] = [];
