...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { fromDate, toDate, assignedAdvisorId, assignedTeamId } = req.query;
      ...
      const contactConditions = [isNull(contacts.deletedAt), accessFilter.whereClause];
      ...
      const enteredCounts =
        stageIds.length > 0
          ? ((await db()
              ...
              .where(and(inArray(pipelineStageHistory.toStage, stageIds), ...dateConditions))
      ...
      const exitedCounts =
        stageIds.length > 0
          ? ((await db()
              ...
              .from(pipelineStageHistory)
              .where(and(inArray(pipelineStageHistory.fromStage, stageIds), ...dateConditions))
      ...
      const currentCounts =
        stageIds.length > 0
          ? ((await db()
              ...
              .from(contacts)
              .where(and(inArray(contacts.pipelineStageId, stageIds), ...contactConditions))
      ...
      if (firstStage && lastStage) {
        ...
        overallConversionRate = startedCount > 0 ? (completedCount / startedCount) * 100 : 0;
      }
