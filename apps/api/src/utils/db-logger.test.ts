...
describe('loggedQuery', () => {
  ...
  it('debería loggear error de query', async () => {
    ...
    await expect(loggedQuery(mockLogger, 'test-query', queryFn, 'select')).rejects.toThrow(
      'DB error'
    );
...
describe('loggedTransaction', () => {
  ...
  it('debería loggear error de transacción', async () => {
    ...
    await expect(loggedTransaction(mockLogger, 'test-transaction', transactionFn)).rejects.toThrow(
      'Transaction failed'
    );
