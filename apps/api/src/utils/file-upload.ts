...
type MulterFileFilterFn = (req: Request, file: Express.Multer.File, callback: multer.FileFilterCallback) => void;
...
export function createMulterUpload(options: FileUploadOptions): multer.Multer {
  ...
  const hasFilter = (options.allowedMimeTypes && options.allowedMimeTypes.length > 0) ||
