...
export async function loggedQuery<T>(
  ...
  try {
    ...
    import('./metrics').then(({ dbQueryDuration, dbQueriesTotal }) => {
    ...
    if (lastQuery && extractOperationBase(lastQuery.operationBase) === extractOperationBase(operation)) {
 ...
 */
function updateAggregatedMetrics(operation: string, duration: number, nPlusOneDetected: boolean): void {
  const operationBase = extractOperationBase(operation);
  const now = Date.now();
  ...
  if (queryMetrics.size > MAX_METRICS_ENTRIES) {
    ...
    const oldest = Array.from(queryMetrics.entries())
...
export function getSlowQueries(thresholdMs: number = 500): AggregatedQueryMetrics[] {
  return getQueryMetrics()
    .filter(m => m.p95Duration > thresholdMs);
}
