...
describe('Query Performance Benchmarks', () => {
  it('should query contacts list in < 100ms', async () => {
    ...
    await db().select().from(contacts).limit(100);
  ...
  it('should query single contact by ID in < 50ms', async () => {
    ...
    await db().select().from(contacts).where(eq(contacts.id, firstContact.id)).limit(1);
  ...
  it('should handle pagination efficiently', async () => {
    ...
    await db().select().from(contacts).limit(10).offset(0);
