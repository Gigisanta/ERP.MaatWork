...
def calculate_rolling_beta(
    ...
    beta = (
        aligned["asset"].rolling(window=window).cov(aligned["benchmark"])
        / aligned["benchmark"].rolling(window=window).var()
    )
...
def calculate_all_risk_metrics(
    ...
    if benchmark_prices is not None:
        ...
        if len(aligned_returns) > 0 and len(aligned_benchmark) > 0:
            ...
            metrics["beta"] = (
                aligned_returns.cov(aligned_benchmark) / aligned_benchmark.var()
                if aligned_benchmark.var() > 0
                else 0.0
            )
            metrics["rolling_beta"] = (
                rolling_beta.iloc[-1]
                if len(rolling_beta) > 0 and not pd.isna(rolling_beta.iloc[-1])
                else None
            )
