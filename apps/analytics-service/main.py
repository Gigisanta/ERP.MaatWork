...
class PortfolioComparisonRequest(BaseModel):
    portfolios: List[
        Dict[str, Any]
    ]  # [{"id": "p1", "name": "Portfolio 1", "components": [...]}]
    period: str = "1Y"
...
async def root():
    """Root endpoint"""
    return {"message": "Cactus Analytics Service", "version": "1.0.0", "docs": "/docs"}
...
async def fetch_current_prices(request: PriceFetchRequest):
    ...
    try:
        if not request.symbols:
            raise HTTPException(
                status_code=400, detail="Lista de símbolos no puede estar vacía"
            )

        if len(request.symbols) > 50:
            raise HTTPException(
                status_code=400, detail="Máximo 50 símbolos por request"
            )
...
async def backfill_historical_prices(request: PriceBackfillRequest):
    ...
    try:
        ...
        if len(request.symbols) > 20:
            raise HTTPException(
                status_code=400, detail="Máximo 20 símbolos para backfill"
            )

        if request.days < 1 or request.days > 3650:  # Máximo 10 años
            raise HTTPException(
                status_code=400, detail="Días debe estar entre 1 y 3650"
            )

        logger.info(
            f"Backfilling {request.days} days for {len(request.symbols)} symbols"
        )
...
async def search_symbols(request: dict):
    ...
    try:
        ...
        logger.error(
            f"Error searching symbols for query '{request.get('query', '')}': {str(e)}"
        )
        raise HTTPException(
...
async def calculate_portfolio_performance(request: PortfolioPerformanceRequest):
    ...
    try:
        ...
        components = [
            PortfolioComponent(
                symbol=comp["symbol"], weight=comp["weight"], name=comp.get("name")
            )
    ...
    try:
        ...
        for portfolio_data in request.portfolios:
            ...
            portfolio_tuples.append(
                (portfolio_data["id"], portfolio_data["name"], components)
            )

        # Calcular rendimiento para todas las carteras
        results = portfolio_calculator.compare_portfolios(
            portfolios=portfolio_tuples, period=request.period
        )
