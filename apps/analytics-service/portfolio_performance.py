...
class PortfolioPerformanceCalculator:
    ...
    def calculate_portfolio_performance(
        ...
        try:
            ...
            if abs(total_weight - 1.0) > 0.01:
                raise ValueError(f"Los pesos deben sumar 1.0, actual: {total_weight:.4f}")
            
            # Obtener datos históricos para todos los componentes
            price_data = self._fetch_historical_data(components, start_date, end_date)
            
            if price_data.empty:
                raise ValueError("No se pudieron obtener datos históricos para los componentes")
            
            # Calcular rendimiento de la cartera
            portfolio_returns = self._calculate_portfolio_returns(price_data, components)
            
            # Calcular métricas
            total_return = self._calculate_total_return(portfolio_returns)
            annualized_return = self._calculate_annualized_return(portfolio_returns, start_date, end_date)
            volatility = self._calculate_volatility(portfolio_returns)
            sharpe_ratio = self._calculate_sharpe_ratio(portfolio_returns, annualized_return)
            max_drawdown = self._calculate_max_drawdown(portfolio_returns)
            
            # Crear serie temporal de rendimiento acumulado
            performance_series = self._create_performance_series(portfolio_returns, start_date)
    ...
    def _calculate_portfolio_returns(self, price_data: pd.DataFrame, components: List[PortfolioComponent]) -> pd.Series:
    ...
    def _calculate_annualized_return(self, returns: pd.Series, start_date: datetime, end_date: datetime) -> float:
    ...
    def _create_performance_series(self, returns: pd.Series, start_date: datetime) -> List[Dict]:
    ...
    def compare_portfolios(
        self,
        portfolios: List[Tuple[str, str, List[PortfolioComponent]]],  # (id, name, components)
        period: str = "1Y"
