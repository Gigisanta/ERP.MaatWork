...
interface HistoricalMetricsStoreState {
  ...
  modal: { isOpen: boolean; metricKey: string; title: string };
}
...
export const useHistoricalMetricsStore = create<HistoricalMetricsStore>()(devtools(
  (set, get) => ({
    metrics: [],
    trends: [],
    ...
    chartData: [],
    modal: { isOpen: false, metricKey: '', title: '' },
    ...
    setViewMode: (mode) => {
      ...
      set({ viewMode: mode, timeRange: newRange });
    },
    setModalOpen: (open) => set((state) => ({ modal: { ...state.modal, isOpen: open } })),
    openModal: (metricKey, title) => set({ modal: { isOpen: true, metricKey, title } }),
    setLoading: (loading) => set({ loading }),
    setError: (error) => set({ error: error ?? undefined }),

    fetchHistoricalMetrics: async (request) => {
      try {
        set({ loading: true, error: undefined });
        const mock: HistoricalMetricsEnhanced[] = [];
        ...
        set({ metrics: mock, loading: false });
      } catch (error) {
        console.error('Error fetching historical metrics:', error);
        set({ error: error instanceof Error ? error.message : 'Error fetching metrics', loading: false });
      }
    },

    calculateTrends: async (metricName, period) => {
      try {
        set({ loading: true });
        const mock: MetricTrend = {
        ...
        const state = get();
        set({ trends: [...state.trends, mock], loading: false });
        return mock;
      } catch (error) {
        console.error('Error calculating trends:', error);
        set({ error: error instanceof Error ? error.message : 'Error calculating trends', loading: false });
        throw error;
      }
    },

    updateRetentionConfig: async (config) => {
      try {
        set({ loading: true });
        ...
        set({ retentionConfig: updated, loading: false });
      } catch (error) {
        console.error('Error updating retention config:', error);
        set({ error: error instanceof Error ? error.message : 'Error updating retention config', loading: false });
      }
    },
    ...
    createAlert: async (alertData) => {
      try {
        set({ loading: true });
        const newAlert: MetricAlert = { ...alertData, id: `alert-${Date.now()}`, created_at: new Date().toISOString(), updated_at: new Date().toISOString() };
        const state = get();
        set({ alerts: [...state.alerts, newAlert], loading: false });
      } catch (error) {
        console.error('Error creating alert:', error);
        set({ error: error instanceof Error ? error.message : 'Error creating alert', loading: false });
      }
    },
    ...
    updateAlert: async (id, updates) => {
      try {
        set({ loading: true });
        const state = get();
        const updatedAlerts = state.alerts.map(a => (a.id === id ? { ...a, ...updates, updated_at: new Date().toISOString() } : a));
        set({ alerts: updatedAlerts, loading: false });
      } catch (error) {
        console.error('Error updating alert:', error);
        set({ error: error instanceof Error ? error.message : 'Error updating alert', loading: false });
      }
    },
    ...
    deleteAlert: async (id) => {
      try {
        set({ loading: true });
        const state = get();
        set({ alerts: state.alerts.filter(a => a.id !== id), loading: false });
      } catch (error) {
        console.error('Error deleting alert:', error);
        set({ error: error instanceof Error ? error.message : 'Error deleting alert', loading: false });
      }
    },
    ...
    navigateToNextPeriod: () => {
      ...
      set({ currentDate: newDate, timeRange: newRange });
      if (state.selectedMetric) get().fetchHistoricalMetrics({ startDate: newRange.start.toISOString(), endDate: newRange.end.toISOString(), granularity: state.viewMode === 'day' ? 'daily' : 'monthly' });
    },
    ...
    resetToCurrentPeriod: () => {
      ...
      set({ currentDate, timeRange: newRange });
      if (state.selectedMetric) get().fetchHistoricalMetrics({ startDate: newRange.start.toISOString(), endDate: newRange.end.toISOString(), granularity: state.viewMode === 'day' ? 'daily' : 'monthly' });
    },

    exportData: async (metricKey, startDate, endDate) => {
      try {
        set({ loading: true });
        const data = await historicalMetricsService.exportHistoricalData(metricKey, startDate, endDate);
        set({ loading: false });
        return data;
      } catch (error) {
        console.error('Error exporting data:', error);
        set({ error: error instanceof Error ? error.message : 'Error exporting data', loading: false });
        throw error;
      }
    },

    clearData: () => set({ metrics: [], chartData: [], trends: [], selectedMetric: '', error: undefined }),

    refreshData: async () => {
      const state = get();
      if (state.selectedMetric && state.timeRange.start && state.timeRange.end) {
        await get().fetchHistoricalMetrics({ startDate: state.timeRange.start.toISOString(), endDate: state.timeRange.end.toISOString(), granularity: state.viewMode === 'day' ? 'daily' : 'monthly' });
      }
    }
  }),
  { name: 'historical-metrics-store', partialize: (state) => ({ viewMode: state.viewMode, currentDate: state.currentDate }) }
));
