...
export async function createAdvisorAlias(data: {
  alias: string;
  userId: string;
}): Promise<ApiResponse<{ alias: AdvisorAliasDto }>> {
  return apiClient.post<{ alias: AdvisorAliasDto }>('/v1/admin/settings/advisors/aliases', data);
}

export async function updateAdvisorAlias(
  id: string,
  data: { alias?: string; userId?: string }
