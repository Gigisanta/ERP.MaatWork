...
describe('DebugConsole', () => {
  ...
  describe('Error handlers', () => {
    it('debería capturar window error events', () => {
      ...
      const errorHandler = (
        global.window.addEventListener as ReturnType<typeof vi.fn>
      ).mock.calls.find((call: unknown[]) => call[0] === 'error')?.[1] as (
        event: ErrorEvent
      ) => void;
    ...
    it('debería capturar unhandled promise rejections', () => {
      ...
      const rejectionHandler = (
        global.window.addEventListener as ReturnType<typeof vi.fn>
      ).mock.calls.find((call: unknown[]) => call[0] === 'unhandledrejection')?.[1] as (
