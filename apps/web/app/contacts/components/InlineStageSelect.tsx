...
const InlineStageSelect = React.memo<InlineStageSelectProps>(
  ({ contact, pipelineStages, isSaving, onStageChange, onMutate, onError }) => {
    ...
    const currentStage = pipelineStages.find(
      (s: PipelineStage) => s.id === contact.pipelineStageId
    );
      ...
      >
        ...
        >
          ...
          <DropdownMenuItem onClick={() => handleStageChange('none')}>
            ...
            {!contact.pipelineStageId && <Icon name="check" size={14} className="ml-auto" />}
          </DropdownMenuItem>
        </DropdownMenu>

        {/* Separador vertical */}
        {nextStage && <div className="h-5 w-px bg-border" />}
