 ...
 */
const ContactListItem = memo(function ContactListItem({ contact, router, index = 0 }: ContactListItemProps) {
  const handleClick = () => {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 md:gap-3">
      {pipelineStages.map((stage: PipelineStage, stageIndex: number) => {
        ...
        return (
          ...
          >
            <Card className="rounded-md border border-border hover:border-primary/20 hover:shadow-sm transition-all" padding="sm">
              ...
              <div className="space-y-1.5">
                {stageContacts.length === 0 ? (
                  ...
                  stageContacts.map((contact: Contact, contactIndex: number) => (
                    <ContactListItem key={contact.id} contact={contact} router={router} index={contactIndex} />
                  ))
                )}
              </div>
            </CardContent>
