    ...
    <div className="space-y-4">
        ...
        <div>
          ...
          {file && (
            <p className="text-sm text-gray-600">
              Archivo: {file.originalFilename} | Estado: {file.status}
            </p>
          )}
        </div>
            ...
            onClick={async () => {
              try {
                ...
                setToast({
                  ...
                  description:
                    error.userMessage || error.message || error.error || 'Error desconocido',
                  variant: 'error',
                });
              }
            }}
      ...
      {file && file.totals && (
        <Text size="sm" color="secondary">
          Procesados: {file.totals.parsed} · Coincidencias: {file.totals.matched} · Pendientes:{' '}
          {file.totals.unmatched}
        </Text>
      )}
      {error && (
            ...
            {rows.map((r) => (
              <tr key={r.id}>
                ...
                <td className="px-4 py-2 text-sm text-gray-700">
                  {String(r.raw?.Titular ?? r.holderName ?? '-')}
                </td>
                <td className="px-4 py-2 text-sm text-gray-700">{r.matchedUserId || '-'}</td>
                <td className="px-4 py-2 text-sm">
                  <Badge variant={r.matchStatus === 'matched' ? 'success' : 'warning'} size="sm">
                    {r.matchStatus}
                  </Badge>
                </td>
                <td className="px-4 py-2 text-sm">
