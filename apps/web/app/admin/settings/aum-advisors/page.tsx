...
export default function AumAdvisorSettingsPage() {
  ...
  const advisorsOptions = useMemo(
    () => advisors.map((a) => ({ value: String(a.id), label: a.fullName || a.email })),
    [advisors]
  );
  ...
  return (
    <div className="p-6">
      <Stack direction="row" justify="between" align="center" className="mb-4">
        <Text as="h1" weight="semibold" size="lg">
          AUM · Advisor Aliases
        </Text>
      ...
      {error && (
        <Text size="sm" className="text-error mb-3">
          {error}
        </Text>
      )}
      ...
      <Card className="mb-6 p-4">
        ...
        <Stack direction="row" gap="sm" align="center">
          ...
          <Select
            ...
            className="w-64"
          />
          <Button onClick={onAdd} disabled={saving || !newAlias || !newUserId}>
            Agregar
          </Button>
        </Stack>
        <Text size="sm" color="secondary" className="mt-2">
          El alias hace match exacto tras normalizar: trim + lowercase.
        </Text>
      </Card>
                ...
                return (
                  <tr key={a.id}>
                    ...
                    <td className="px-4 py-2 text-sm text-gray-700">
                      {user ? user.fullName || user.email : a.userId}
                    </td>
