...
function formatReport(combined) {
  ...
  console.log('\nTOTAL:');
  console.log(
    `  Lines:      ${combined.total.lines.pct.toFixed(2)}% (${combined.total.lines.covered}/${combined.total.lines.total})`
  );
  console.log(
    `  Statements: ${combined.total.statements.pct.toFixed(2)}% (${combined.total.statements.covered}/${combined.total.statements.total})`
  );
  console.log(
    `  Functions:  ${combined.total.functions.pct.toFixed(2)}% (${combined.total.functions.covered}/${combined.total.functions.total})`
  );
  console.log(
    `  Branches:   ${combined.total.branches.pct.toFixed(2)}% (${combined.total.branches.covered}/${combined.total.branches.total})`
  );
  console.log('═'.repeat(60) + '\n');
}
