...
function checkPostgreSQL() {
  ...
  if (dockerResult.success && dockerResult.output) {
    console.log(
      success(`  ✅ PostgreSQL corriendo en Docker: ${dockerResult.output.split('\n')[0]}`)
    );
    return true;
  }
  ...
  if (isWindows) {
    ...
    pgCheckCommand =
      'pg_isready -h localhost -U postgres 2>/dev/null || pg_isready -h localhost 2>/dev/null';
  }
  ...
  if (dockerComposeResult.success) {
    ...
    console.log(
      warning('     El script intentará iniciarlo automáticamente si Docker está disponible')
    );
    hasWarnings = true;
    return false;
  }
  ...
  if (pythonFound) {
    ...
    if (pipFound) {
      ...
      if (allInstalled) {
        ...
        console.log(
          warning('     Instala dependencias: pnpm -F @cactus/analytics-service install')
        );
        hasWarnings = true;
      }
    } else {
    ...
    console.log(
      warning('     El API usará fallback a base de datos si el servicio no está disponible')
    );
    hasWarnings = true;
    return false;
  }
...
function main() {
  ...
  const criticalChecks = [checkNodeVersion, checkPnpm, checkPostgreSQL];

  // Validaciones opcionales (solo en modo completo)
  const optionalChecks = fastMode ? [] : [checkDocker, checkPython];
