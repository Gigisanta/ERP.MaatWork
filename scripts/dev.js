...
function ensureDockerServices() {
  try {
    ...
    try {
      ...
      console.log(
        warning('⚠️  Docker no está disponible, saltando verificación de servicios Docker\n')
      );
      return;
    }
    ...
    try {
      ...
      if (dockerComposePsOutput.trim()) {
        const containers = dockerComposePsOutput
          .trim()
          .split('\n')
    ...
    if (!postgresRunning || !n8nRunning) {
      console.log(
        warning('⚠️  Servicios Docker no detectados, intentando iniciar con Docker Compose...')
      );
      try {
...
async function runValidations(skipCache = false) {
  ...
  try {
    ...
    if (!fs.existsSync(envPath)) {
      ...
      console.log(warning('   Ejecuta: pnpm setup para configurar el proyecto'));
      console.log(
        warning('   O copia manualmente: cp apps/api/config-example.env apps/api/.env\n')
      );
...
async function main() {
  ...
  if (isWindows) {
    ...
    if (fs.existsSync(unifiedScript)) {
      ...
      console.log(
        warning('⚠️  Script unificado no encontrado. Usando modo básico (turbo run dev)...')
      );
      console.log('');
