...
async function getTableStats(tableName: string, dateColumn: string): Promise<TableStats> {
  ...
  const yearsOfData = minDate && maxDate 
    ? (new Date(maxDate).getTime() - new Date(minDate).getTime()) / (1000 * 60 * 60 * 24 * 365)
    : 0;

  const partitionsRecommended = rowCount > 1_000_000 || totalSizeBytes > 10_000_000_000 || yearsOfData > 2;
...
async function evaluatePartitioning(): Promise<void> {
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (!(tableExists.rows[0] as { exists: boolean }).exists) {
        logger.debug({ table: candidate.tableName }, `Tabla ${candidate.tableName} no existe, omitiendo`);
        continue;
      }
  ...
  if (recommended.length > 0) {
    ...
    recommended.forEach(r => {
      ...
      console.log(`   Estrategia: ${r.partitionStrategy} (por ${r.partitionStrategy === 'monthly' ? 'mes' : 'trimestre'})`);
  ...
  if (recommended.length === 0) {
    ...
    recommended.forEach(r => {
      ...
      console.log(`   - Particionar por ${r.partitionStrategy === 'monthly' ? 'mes' : 'trimestre'}`);
