...
export class CactusStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: CactusStackProps) {
    ...
    const [dbInstanceClass, dbInstanceSize] = config.database.instanceType.split('.') as [
      string,
      string,
    ];
    ...
    this.database = new rds.DatabaseInstance(this, 'Database', {
      ...
      vpc: this.vpc,
      vpcSubnets: {
        subnetType: config.vpc.useDefault ? ec2.SubnetType.PUBLIC : ec2.SubnetType.PRIVATE_ISOLATED,
      },
      ...
      removalPolicy:
        config.environment === 'prod' ? cdk.RemovalPolicy.SNAPSHOT : cdk.RemovalPolicy.DESTROY,
