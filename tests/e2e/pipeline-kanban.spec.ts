...
test.describe('Pipeline Kanban E2E', () => {
  ...
  test('debería mover contacto entre stages con drag & drop', async ({ page }) => {
    ...
    const contactCard = page.locator('[draggable="true"], [data-testid*="contact"], [class*="contact-card"]').first();
    
    if (await contactCard.count() > 0) {
      // Buscar un stage destino
      const targetStage = page.locator('[data-testid*="stage"], [class*="stage"], [class*="column"]').nth(1);
