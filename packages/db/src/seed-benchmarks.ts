...
export async function seedBenchmarks() {
  ...
  const instrumentsData = [
    ...
    {
      symbol: 'GD30D.BA',
      name: 'Bonos Ley Argentina 2030 USD',
      assetClass: 'fixed_income',
      currency: 'USD',
      isin: 'ARARGE3202A0',
      active: true,
  ...
  const insertedBenchmarks = await db()
    ...
    .values(benchmarksData)
    .returning();

  // 4. Seed benchmark components
  // numeric() en Drizzle se representa como string, no como number
  const benchmarkComponentsData = [
    // MERVAL (100% MERVAL index)
    {
      benchmarkId: insertedBenchmarks.find(
        (b: InferSelectModel<typeof benchmarkDefinitions>) => b.code === 'MERVAL'
      )!.id,
      instrumentId: insertedInstruments.find(
        (i: InferSelectModel<typeof instruments>) => i.symbol === '^MERV'
      )!.id,
      weight: '1.0000',
    },
    ...
    {
      benchmarkId: insertedBenchmarks.find(
        (b: InferSelectModel<typeof benchmarkDefinitions>) => b.code === 'SP500'
      )!.id,
      instrumentId: insertedInstruments.find(
        (i: InferSelectModel<typeof instruments>) => i.symbol === '^GSPC'
      )!.id,
      weight: '1.0000',
    },
    ...
    {
      benchmarkId: insertedBenchmarks.find(
        (b: InferSelectModel<typeof benchmarkDefinitions>) => b.code === 'MSCI_EM'
      )!.id,
      instrumentId: insertedInstruments.find(
        (i: InferSelectModel<typeof instruments>) => i.symbol === 'EEM'
      )!.id,
      weight: '1.0000',
    },
    ...
    {
      benchmarkId: insertedBenchmarks.find(
        (b: InferSelectModel<typeof benchmarkDefinitions>) => b.code === 'BALANCED_AR'
      )!.id,
      instrumentId: insertedInstruments.find(
        (i: InferSelectModel<typeof instruments>) => i.symbol === '^MERV'
      )!.id,
    ...
    },
    {
      ...
      instrumentId: insertedInstruments.find(
        (i: InferSelectModel<typeof instruments>) => i.symbol === 'AL30D.BA'
      )!.id,
      weight: '0.4000',
    },
    ...
    {
      benchmarkId: insertedBenchmarks.find(
        (b: InferSelectModel<typeof benchmarkDefinitions>) => b.code === 'CONSERVATIVE_AR'
      )!.id,
      instrumentId: insertedInstruments.find(
        (i: InferSelectModel<typeof instruments>) => i.symbol === '^MERV'
      )!.id,
    ...
    {
      benchmarkId: insertedBenchmarks.find(
        (b: InferSelectModel<typeof benchmarkDefinitions>) => b.code === 'AGGRESSIVE_AR'
      )!.id,
      instrumentId: insertedInstruments.find(
        (i: InferSelectModel<typeof instruments>) => i.symbol === '^MERV'
      )!.id,
