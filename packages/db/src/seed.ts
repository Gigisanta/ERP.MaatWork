...
import { pipelineStages, notificationTemplates } from './schema';
...
async function seed() {
  ...
  try {
    // 1. Crear etapas de pipeline iniciales
    console.log('📊 Creando etapas de pipeline...');
    const pipelineStagesData = [
      {
        name: 'Lead',
        description: 'Contacto inicial sin calificar',
        order: 1,
        color: '#94A3B8',
        wipLimit: null,
        slaHours: 48,
      ...
      {
        name: 'Calificado',
        description: 'Lead calificado con potencial',
        order: 2,
        color: '#3B82F6',
        wipLimit: 20,
        slaHours: 72,
      ...
      {
        name: 'Propuesta',
        description: 'Propuesta presentada al cliente',
        order: 4,
        color: '#F59E0B',
        wipLimit: 10,
        slaHours: 120,
      ...
      {
        name: 'Inactivo',
        description: 'Cliente sin movimientos',
        order: 7,
        color: '#6B7280',
        wipLimit: null,
        slaHours: null,
    ...
    for (const stage of pipelineStagesData) {
      // Verificar si ya existe (omitir verificación en SQLite por simplicidad)
      try {
