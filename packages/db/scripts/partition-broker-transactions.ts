...
async function partitionBrokerTransactions() {
  ...
  try {
    ...
    if (!minDate || !maxDate || totalCount === 0) {
      console.log('⚠️  No hay datos en broker_transactions. Creando particiones futuras solamente...\n');
    } else {
    ...
    while (currentDate <= endDate) {
      ...
      console.log(`   ✓ Partición creada: ${partitionName} (${partitionStart.toISOString().split('T')[0]} a ${partitionEnd.toISOString().split('T')[0]})`);
      partitionCount++;
    ...
    await client.query('ROLLBACK');
    console.error('\n❌ Error durante el particionamiento:', error instanceof Error ? error.message : String(error));
    throw error;
  } finally {
    client.release();
  }
